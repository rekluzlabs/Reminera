...
**Status:** Early Alpha — core capture/import flows working, offline raw assembly + optional Gemini AI polish built (Step 4 of 5 for PDF/QR export). No functionality gated behind having a Gemini key.
...
| Database | Room (version 10, schema exported) |
| Camera | CameraX |
...
| PDF Merge | PDFBox-Android (`com.tom-roush:pdfbox-android:2.0.27.0`) |
| Secure Storage | AndroidX Security Crypto (`EncryptedSharedPreferences` with Keystore fallback) |
...
- [x] Room database set up (`RemineraDatabase`, version 10, schema export enabled)
...
| `ChapterExportEntity` | `chapter_exports` | memberId (PK), groupId, sourceDataHash, generatedBioText, mediaManifestJson, lastGenerated, renderedPdfPath, renderedPdfHash, biographySource, aiPolishedAt |
...
| `ChapterExportDao` | getByMemberId, upsert, deleteByMemberId, updateRenderedPdf, updateBiographySource, updateAiPolished |
...
- [x] **Chapter export caching data layer** (Step 1 of 5):
  ...
  - [x] `ChapterExportRepository.getOrGenerateChapter()` — cache-check with raw text assembly (via `RawChapterTextAssembler` since Step 4)
...
- [x] **Book assembly + PDFBox-Android merge + TOC + page numbers** (Step 3 of 5):
  - [x] `ChapterExportEntity` extended with `renderedPdfPath`/`renderedPdfHash` for PDF render caching
  - [x] `MIGRATION_8_9` — adds `renderedPdfPath` and `renderedPdfHash` columns to `chapter_exports`, DB bumped to v9
  - [x] `ChapterRenderCacheRepository.ensureRenderedChapter()` — skips re-render when hash matches and file exists
  - [x] PDFBox-Android (`com.tom-roush:pdfbox-android:2.0.27.0`) added as dependency
  - [x] `RemineraApplication` — custom Application class calling `PDFBoxResourceLoader.init()` at startup
  - [x] `TocHtmlTemplateBuilder` — Table of Contents HTML template reusing Step 2's CSS variable pattern
  - [x] `BookAssembler.assembleBook(manifestId)` — merges chapters, iterative TOC page-offset correction (capped at 3 iterations), PDF outline bookmarks, page-number overlay
  - [x] Output saved to `context.filesDir/exports/books/{manifestId}_{title-slug}.pdf`
  - [x] All `PDDocument` instances closed in `finally` blocks
  - [x] `BookAssemblerTest` — 11 tests covering TOC page-offset convergence scenarios
  - [x] `ChapterRenderCacheRepositoryTest` — 5 tests covering render-cache freshness logic (hash match, file missing, hash mismatch, no prior render)
- [x] **Offline raw assembly + optional Gemini AI polish** (Step 4 of 5):
  - [x] `RawChapterTextAssembler` — pure, offline, always-succeeds text assembly from member biography + sections + stories (double-newline-separated paragraphs matching `ChapterHtmlTemplateBuilder` expectations)
  - [x] `MIGRATION_9_10` — adds `biographySource` ("RAW"/"AI_POLISHED") and `aiPolishedAt` to `chapter_exports`, DB bumped to v10
  - [x] `getOrGenerateChapter()` updated: hash match returns cached (either source); hash mismatch always produces `RAW` text, never calls Gemini
  - [x] `BiographyGenerationProvider` interface + `BiographyGenerationInput` (text-only, no media)
  - [x] `GeminiBiographyProvider` — plain-prose-only prompt, 20k char input length cap, `HttpURLConnection` to Gemini API
  - [x] `requestAiPolish()` — explicit opt-in call on `ChapterExportRepository`, success updates all fields together, failure leaves entity untouched
  - [x] `SecureApiKeyStore` — `EncryptedSharedPreferences` with Keystore-corruption fallback (catch → clear → recreate)
  - [x] `AiConsentDialog` — one-time consent shown only on first AI-polish action, persisted as SharedPreferences boolean, resettable from Settings
  - [x] `AiSettingsScreen` — password-masked API key field, Save/Clear, Reset Consent
  - [x] Settings row "AI Biography (Gemini)" + navigation
  - [x] `BiographyScreen` — source indicator ("Raw" / "AI-polished") + Generate/Regenerate button
  - [x] `BiographyViewModel` — `chapterExport` state, `isAiPolishing` loading state, `requestAiPolish()` method
  - [x] `RawChapterTextAssemblerTest` — 10 tests (block ordering, blank-line separation, empty sections/stories, whitespace trimming)
  - [x] `GeminiBiographyProviderTest` — 6 tests (prompt building, input truncation at boundary/over-cap)
  - [x] `ChapterExportRepositoryAiPolishTest` — 6 tests (success updates fields, failure untouched, hash-change reverts to raw)
  - [x] `ChapterExportRepositoryTest` updated — 8 tests (raw-assembled text, hash-match-cached regardless of source, hash-mismatch-raw-only)
- [ ] QR code generation + hosting — not started (Step 5)
...
- [x] Room migration strategy — `MIGRATION_6_7` for sortOrder, `MIGRATION_7_8` for export tables, `MIGRATION_8_9` for PDF render cache fields, `MIGRATION_9_10` for AI polish tracking; earlier versions use `fallbackToDestructiveMigration(dropAllTables = true)`
- [ ] **Backup/restore or data export** — not implemented. Settings screen has theme + AI key entry.
...
- [x] `ChapterExportRepositoryTest` — 8 tests covering cache hit, bio edit invalidation, media reorder invalidation, manifest JSON, raw-assembled text, hash-match returns cached regardless of source, hash-mismatch produces raw text
...
- [x] `BookAssemblerTest` — 11 tests covering TOC page-offset convergence (single/multi chapter, various TOC sizes, edge cases)
- [x] `ChapterRenderCacheRepositoryTest` — 5 tests covering render-cache freshness logic (hash match, file missing, hash mismatch, no prior render)
- [x] `RawChapterTextAssemblerTest` — 10 tests covering block ordering, blank-line separation, empty sections/stories, whitespace trimming, non-text stories skipped
- [x] `GeminiBiographyProviderTest` — 6 tests covering prompt building, input truncation at boundary/over-cap
- [x] `ChapterExportRepositoryAiPolishTest` — 6 tests covering success updates fields, failure entity untouched, hash-change reverts to raw
...
- [x] Settings screen exists (theme selector + AI biography key entry + branding)
...
4. **`fallbackToDestructiveMigration` is active** — Only migrations `6→7`, `7→8`, `8→9`, and `9→10` are explicit. Any schema change on versions 1–5 drops all data. Acceptable for alpha, but must be fixed before users have real data.

5. **Placeholder text in chapter exports** — ~~`ChapterExportRepository.getOrGenerateChapter()` generates placeholder biography text marked with `TODO(step-4)`. This is intentional for Step 1; real AI generation comes in Step 4.~~ **Resolved in Step 4.** `RawChapterTextAssembler` produces real assembled text from member bio + sections + stories. `getOrGenerateChapter()` never calls Gemini — AI polish is a separate, explicit `requestAiPolish()` action.
...
8. **Settings is bare** — Only theme + AI key entry + branding. Missing: backup/restore, about, privacy info, version details, storage usage.
