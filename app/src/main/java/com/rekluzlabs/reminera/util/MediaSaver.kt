...
object MediaSaver {
    ...
    fun saveStoryEntryToDevice(context: Context, entry: StoryEntryEntity): Boolean {
        val uriStr = entry.mediaUri ?: return false
        val sourceFile = File(uriStr)
        if (!sourceFile.exists()) return false

        return try {
            val typeUpper = entry.type.uppercase()
            val mimeType = when (typeUpper) {
                "PHOTO" -> "image/jpeg"
                "VIDEO" -> "video/mp4"
                "AUDIO" -> "audio/mp4"
                else -> "application/octet-stream"
            ...
            val displayName = "Story_${entry.type}_${entry.id.take(8)}.$extension"
            ...
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q) {
                ...
                val collectionUri = when (typeUpper) {
                    "PHOTO" -> MediaStore.Images.Media.EXTERNAL_CONTENT_URI
                    "VIDEO" -> MediaStore.Video.Media.EXTERNAL_CONTENT_URI
                    "AUDIO" -> MediaStore.Audio.Media.EXTERNAL_CONTENT_URI
                    else -> MediaStore.Downloads.EXTERNAL_CONTENT_URI
