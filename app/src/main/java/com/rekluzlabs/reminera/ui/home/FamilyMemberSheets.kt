...
fun AddFamilyMemberSheet(
    ...
    val sheetState = rememberModalBottomSheetState(
        ...
        confirmValueChange = { editingPhotoUri == null }
    )
    ...
    editingPhotoUri?.let { uri ->
        Dialog(
            onDismissRequest = { editingPhotoUri = null },
...
fun EditFamilyMemberSheet(
    member: com.rekluzlabs.reminera.data.FamilyMemberEntity,
    groupName: String,
    groupType: String,
    onSave: (name: String, role: String, birthDate: Long?, photoUri: String?) -> Unit,
    onDismiss: () -> Unit
        ...
        Column(
            ...
            Card(
                ...
                Row(
                    ...
                    Spacer(modifier = Modifier.width(10.dp))
                    val hasNewPhoto = photoUri != null && photoUri.toString() != member.photoUri
                    Text(
                        text = if (hasNewPhoto) "New photo selected" else if (member.photoUri != null) "Change photo" else "Add photo (optional)",
                        color = MaterialTheme.colorScheme.onSurfaceVariant,
                        fontSize = 15.sp
                    )
                }
